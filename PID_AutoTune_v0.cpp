...
int PID_ATune::Runtime()
{
	...
	if(refVal>setpoint+noiseBand){
		*output = outputStart-oStep;
		Serial.print("autotune temp over:");Serial.println(*output);
	} else if (refVal<setpoint-noiseBand){
		*output = outputStart+oStep;
		Serial.print("autotune temp under:");Serial.println(*output);
	}
  ...
  if(isMax)
  {
	  Serial.print("IsMax ");Serial.println(peakType);
  ...
  {
	  Serial.print("IsMin ");Serial.println(peakType);
    if(peakType==0)peakType=-1;
    if(peakType==1)
    {
      peakType=-1;
  ...
  Serial.print("peakCount: ");Serial.println(peakCount);
...
void PID_ATune::SetLookbackSec(int value)
{
	...
	Serial.println("autoTune sampleTime: ");Serial.println(sampleTime);
}
